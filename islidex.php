...
function islidex_add_menu() {
	add_options_page('iSlidex', 'iSlidex', 'administrator', __FILE__, 'islidex_options_page'); 
}
...
<h2>iSlidex Settings</h2>
	...
	<tr valign="top">
		...
		<!-- <option value="3" <?php check_sel(3,1); ?>>Horizontal Accordion</option> -->
		</select></td>
	</tr>
...
function islidexcss() {
		...
		$hozaccss_path = ISLIDEX_PLUGIN_CSS . '/islidex_hozac.css';
	...
	function islidexjs() {
		...
		if ($islidex_options['theme'] == 1 ) { // the Apple theme
			...
			//customize the slider further by editing islidex.php in the plugin folder
		...
		} elseif ($islidex_options['theme'] == 3) { // the Horizontal Accordion theme
			echo '<!-- iSlidex JS Dependencies -->
			<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.min.js"></script>
			<script type="text/javascript" src="'.$hozac_easing_path.'"></script>
			<script type="text/javascript" src="'.$hozac_path.'"></script>
			<script type="text/javascript">
			$(document).ready(function() {
			$(".islidex_content").hrzAccordion({
			eventTrigger	   		: "click",
			containerClass     		: "islidex_container",
			listItemClass      		: "islidex_listItem",					
...
function show_islidex() {
	...
	//	global $post;
		...
		$attachments = get_children( array('post_parent' => $islidex_post->ID, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'image', 'orderby' => 'rand', 'numberposts' => 1) );
		if ($slide == true) { 
			echo '<a href="'.get_permalink($islidex_post->ID).'"><img src="'.$timthumb_path.'?src='.$slide.'&w='.$islidex_options['slide_size_w'].'&h='.$islidex_options['slide_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a>';
			} else if ($attachments == true) {
				foreach($attachments as $id => $attachment) {
				$img = wp_get_attachment_image_src($id, 'full');
				$img_url = parse_url($img[0], PHP_URL_PATH);
				print '<a href="'.get_permalink($islidex_post->ID).'"><img src="'.$timthumb_path.'?src='.$img_url.'&w='.$islidex_options['slide_size_w'].'&h='.$islidex_options['slide_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a>';
			}
		}
	}
wp_reset_query(); ?>
	...
	<div class="islidex_hozac" style="height:<?php echo $islidex_options['slide_size_h'] ?>px;width:<?php echo $islidex_options['slide_size_w'] ?>px;">
	<div class="islidex_container" style="min-height:<?php echo $islidex_options['slide_size_h'] ?>px;min-width:<?php echo $islidex_options['slide_size_w'] ?>px;width:<?php echo $islidex_options['slide_size_w'] ?>px;height:<?php echo $islidex_options['slide_size_h'] ?>px;">
	<ul class="islidex_content">
	...
	//	global $post;
		...
		$attachments = get_children( array('post_parent' => $islidex_post->ID, 'post_status' => 'inherit', 'post_type' => 'attachment', 'post_mime_type' => 'image', 'orderby' => 'rand', 'numberposts' => 1) );
		if ($slide == true) {
		echo '<li><div class="handle"></div><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$islidex_options['slide_size_w'].'" height="'.$islidex_options['slide_size_h'].'" src="'.$timthumb_path.'?src='.$slide.'&w='.$islidex_options['slide_size_w'].'&h='.$islidex_options['slide_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></li>';
		} else if ($attachments == true) {
			foreach($attachments as $id => $attachment) {
			$img = wp_get_attachment_image_src($id, 'full');
			$img_url = parse_url($img[0], PHP_URL_PATH);
			print '<li><div class="handle"></div><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$islidex_options['slide_size_w'].'" height="'.$islidex_options['slide_size_h'].'" src="'.$timthumb_path.'?src='.$img_url.'&w='.$islidex_options['slide_size_w'].'&h='.$islidex_options['slide_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></li>';
			}
		}
	} ?>
	</ul>
...
function widget_islidex_init() {
	...
	function widget_islidex($args) {
		...
		<div class="gallery" id="gallery" style="width:<?php echo $islidex_options['widget_size_w']; ?>px;">
			<div id="slides" style="height:<?php echo $islidex_options['widget_size_h']; ?>px;">
		...
		//	global $post;
			...
			if ($slide == true) {
				echo '<div class="slide"><a href="'.get_permalink($islidex_widget->ID).'"><img width="'.$islidex_options['widget_size_w'].'" height="'.$islidex_options['widget_size_h'].'" src="'.$timthumb_path.'?src='.$slide.'&w='.$islidex_options['widget_size_h'].'&h='.$islidex_options['widget_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a></div>';
				}else if ($attachments == true) {
					foreach($attachments as $id => $attachment) {
					$img = wp_get_attachment_image_src($id, 'full');
					$img_url = parse_url($img[0], PHP_URL_PATH);
					print '<div class="slide"><a href="'.get_permalink($islidex_widget->ID).'"><img width="'.$islidex_options['widget_size_w'].'" height="'.$islidex_options['widget_size_h'].'" src="'.$timthumb_path.'?src='.$img_url.'&w='.$islidex_options['widget_size_h'].'&h='.$islidex_options['widget_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a></div>';
				}
			} 
		} 
wp_reset_query(); ?>
