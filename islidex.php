...
function islidexcss() {
	...
	function islidexjs() {
		...
		if (($islidex_options['usecaption']) == 1) : $cap = "true"; endif;
...
} elseif (($islidex_options['theme']) == 'Timeline') {  // THEME 4 - TIMELINE ?>
...
function show_customislidex ($customcatid,$customnumpost,$width,$height,$customtheme) {
	...
	foreach($slideposts as $islidex_post) {
		...
		if (($islidex_options['linked']) == 1): echo '<a href="'.get_permalink($islidex_post->ID).'">'; endif;
		print '<a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.ISLIDEX_PLUGIN_IMAGES.'/wp_big.png&w=250&h=250&zc=0&q=100" alt="'.$title.'" title="'.$title.'" style="padding-top:15%;" />';
		if (($islidex_options['linked']) == 1): echo '</a>'; endif;
		}
	} 
	wp_reset_query(); ?>
...
<?php echo $piece_prep; 
	...
	<div class="timeline" id="timeline" style="min-height:<?php echo $height; ?>px;min-width:<?php echo $width; ?>px;width:<?php echo $width; ?>px;">
	<div id="times" style="height:<?php echo $height; ?>px;">
	...
	foreach($slideposts as $islidex_post) {
		...
		if (function_exists('has_post_thumbnail') && has_post_thumbnail($islidex_post->ID)) {
			...
			echo '<div class="time"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.$time.'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></div>';
		} else if ($attachments == true) {
			foreach($attachments as $id => $attachment) {
			$img = wp_get_attachment_image_src($id, 'full');
			$img_url = parse_url($img[0], PHP_URL_PATH);
		...
		print '<div class="time" style="height: '.$height.'px; width: '.$width.'px;">';
			if (($islidex_options['linked']) == 1): echo '<a href="'.get_permalink($islidex_post->ID).'">'; endif;
			print '<img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.ISLIDEX_PLUGIN_IMAGES.'/wp_big.png&w=250&h=250&zc=0&q=100" alt="'.$title.'" title="'.$title.'" class="captify" style="padding-top:15%;" />';
			if (($islidex_options['linked']) == 1): echo '</a>'; endif;
			echo '</div>';
	...
	class islidex_widget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			foreach($slideposts as $islidex_widget) {
				...
				if (($islidex_options['wlinked']) == 1): echo '<a href="'.get_permalink($islidex_post->ID).'">'; endif;
