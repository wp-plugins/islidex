...
function islidexcss() {
	...
	function islidexjs() {
		...
		if (($islidex_options['usecaption']) == 1 && ($islidex_options['theme']) == 'Apple') {
			$cap = "false";
		} elseif (($islidex_options['usecaption']) == 1 && ($islidex_options['theme']) == 'Nivo') {
...
function show_islidex() {
	...
	foreach($slideposts as $islidex_post) {
		...
		if (function_exists('has_post_thumbnail') && has_post_thumbnail($islidex_post->ID)) {
			...
			$img = wp_get_attachment_image_src($id, 'full');
			$img_url = parse_url($img[0], PHP_URL_PATH);
			print '<a href="'.get_permalink($islidex_post->ID).'"><img width="'.$islidex_options['slide_size_w'].'" height="'.$islidex_options['slide_size_h'].'" src="'.$timthumb_path.'?src='.$img_url.'&w='.$islidex_options['slide_size_w'].'&h='.$islidex_options['slide_size_h'].'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></a>';
			}
		} else {
		print '<a href="'.get_permalink($islidex_post->ID).'"><img width="'.$islidex_options['slide_size_w'].'" height="'.$islidex_options['slide_size_h'].'" src="'.$timthumb_path.'?src='.ISLIDEX_PLUGIN_IMAGES.'/wp_big.png&w=250&h=250&zc=0&q=100" alt="'.$title.'" title="'.$title.'" style="padding-top:15%;" /></a>';
		}
	} 
	wp_reset_query(); ?>
	</div>
	...
	<script type="text/javascript">
			...
			swfobject.embedSWF("'.$piece_swf.'", "flashcontent", "'.($islidex_options['slide_size_w']+50).'", "'.($islidex_options['slide_size_h']+180).'", "10", "'.$swf_exp.'", flashvars, attributes);
	</script>'; ?>
	<div style="width:<?php ($islidex_options['slide_size_w']+80) ?>px;height:<?php ($islidex_options['slide_size_h']+150) ?>px;vertical-align:middle;text-align:center;margin:0;padding-top:25px;valign:middle;align:middle;display:block;">
	<div id="flashcontent">
...
<?php echo $piece_prep; } // end of Theme 3 and THEMES elseifs, for now
...
function show_customislidex ($customcatid,$customnumpost,$width,$height,$customtheme) {
	...
	foreach($slideposts as $islidex_post) {
		...
		if (function_exists('has_post_thumbnail') && has_post_thumbnail($islidex_post->ID)) {
			$image_id = get_post_thumbnail_id($islidex_post->ID);
			$feat = wp_get_attachment_image_src($image_id,'large', true);
			echo '<div class="slidec"><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.$feat[0].'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a></div>'; // the featured image
		} elseif ($slide == true) {
			echo '<div class="slidec"><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.$slide.'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a></div>';
		} else if ($attachments == true) {
			foreach($attachments as $id => $attachment) {
			$img = wp_get_attachment_image_src($id, 'full');
			$img_url = parse_url($img[0], PHP_URL_PATH);
			print '<div class="slidec"><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.$img_url.'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" class="captify" /></a></div>';
			}
		} else {
		print '<div class="slidec" style="height: '.$height.'px; width: '.$width.'px;"><a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.ISLIDEX_PLUGIN_IMAGES.'/wp_big.png&w=250&h=250&zc=0&q=100" alt="'.$title.'" title="'.$title.'" class="captify" style="padding-top:15%;" /></a></div>';
		}
	} 
	wp_reset_query(); ?>
	</div>
	...
	foreach($slideposts as $islidex_post) {
		...
		if (function_exists('has_post_thumbnail') && has_post_thumbnail($islidex_post->ID)) {
			$image_id = get_post_thumbnail_id($islidex_post->ID);
			$feat = wp_get_attachment_image_src($image_id,'large', true);
			echo '<a href="'.get_permalink($islidex_post->ID).'"><img src="'.$timthumb_path.'?src='.$feat[0].'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></a>'; // the featured image
		} elseif ($slide == true) {
			echo '<a href="'.get_permalink($islidex_post->ID).'"><img src="'.$timthumb_path.'?src='.$slide.'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></a>';
		} else if ($attachments == true) {
			foreach($attachments as $id => $attachment) {
			$img = wp_get_attachment_image_src($id, 'full');
			$img_url = parse_url($img[0], PHP_URL_PATH);
			print '<a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.$img_url.'&w='.$width.'&h='.$height.'&zc=1&q=100" alt="'.$title.'" title="'.$title.'" /></a>';
			}
		} else {
		print '<a href="'.get_permalink($islidex_post->ID).'"><img width="'.$width.'" height="'.$height.'" src="'.$timthumb_path.'?src='.ISLIDEX_PLUGIN_IMAGES.'/wp_big.png&w=250&h=250&zc=0&q=100" alt="'.$title.'" title="'.$title.'" style="padding-top:15%;" /></a>';
		}
	} 
	wp_reset_query(); ?>
	</div>
	...
	<div style="width:<?php echo ($width+50) ?>px;height:<?php echo ($height+100) ?>px;vertical-align:middle;text-align:center;margin:0;padding-top:25px;valign:middle;align:middle;display:block;">
...
function custom_islidex_params($atts) {
	...
	return show_customislidex($cat,$num,$w,$h,$customtheme);
