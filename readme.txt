...
Tags: slideshow, slider, featured, slideshow, slider, carousel, showcase, islidex, captions, nivo, piecemaker, apple, gallery
...
These numbers represent, in order: `cat`egory (where to get the posts from), max `num`ber of posts/slides (to show in iSlidex), `w`idth (of the slider), `h`eight (of the slider), `theme` (1 is Apple, 2 is Nivo, 3 is Piecemaker).
...
= Why can't I choose specific effects (Nivo, Piecemaker themes) or captions in the custom iSlidex?

For now, the custom iSlidex will follow the standard one for what concernces effects and caption selection.
The reason is simple, this is a free plugin and we only have limited time to work on it.
In the next releases we might give the ability to choose the effect and captions.
...
2. Nivo theme
3. Piecemaker theme
4. How to add the custom fields inside posts
