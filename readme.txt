...
The plugin has a simple settings page, it can be used with the shortcodes [islidex_page] and [islidex_post] respectively in pages and posts, or using the php functions `<?php show_islidex(); ?>` and `<?php show_islidexpage(); ?>`, respectively in your template code or using the plugin PHP Shortcode.
...
Credits to TutorialZine for releasing the base script.
...
1. The plugin has a simple settings page, it can be used with the shortcodes [islidex_page] and [islidex_post] respectively in pages and posts, or using the php functions `<?php show_islidex(); ?>` and `<?php show_islidexpage(); ?>`, respectively in your template code or using the plugin [PHP Shortcode](http://wordpress.org/extend/plugins/php-shortcode) to render php functions within posts and pages.
If you would like to customize the look and feel of iSlidex, you can upload your own islidex.css to your own template folder and last but not least, you may also use 2 custom fields in your post, named `islidex_slide` and `islidex_thumb` to have full control over your slider.

== Frequently Asked Questions ==

None yet. Feel free to open a new thread in Wordpress Forums with tag `islidex`.
You can also leave a comment at the official plugin page on [Shambix](http://www.shambix.com/news/wordpress-plugin-islidex) site.

= Why aren't there any questions? =

Cos it's quite easy to make it work :)
